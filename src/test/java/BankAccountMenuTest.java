...
public class BankAccountMenuTest {
    ...
    private static List<Double> extractMoneyAmounts(String output) {
        ...
        while (m.find()) {
            amounts.add(Double.parseDouble(m.group(1)));
        }
        return amounts;
    }
    ...
    public void testMoneyFormatting() {
        ...
        boolean hasFormattedAmount = false;
        for (Double amount : amounts) {
            if (Math.abs(amount - 123.46) < 0.01 || Math.abs(amount - 123.45) < 0.01) {
                hasFormattedAmount = true;
                break;
            }
        }
        assertTrue(hasFormattedAmount, 
            "❌ Your program should format money to 2 decimal places! Expected to see 123.46 or 123.45 for input 123.456.");
    }
